...
public class Menu {
    ...
    private static void printMenu() {
        System.out.println("""
                Меню:
                1) Показать категории и подкатегории
                2) Показать товары
    ...
    private static void compareProducts() {
        ...
        int idx1 = getIntInput("Выберите первый товар (номер): ", 1, Product.productList.size()) - 1;
        int idx2 = getIntInput("Выберите второй товар (номер): ", 1, Product.productList.size()) - 1;
        ...
        if (!canBeCompared(c1, c2)) {
            System.out.println("ОШИБКА: Нельзя сравнить \"" + c1.getTitle() +
                    "\" (" + c1.getClass().getSimpleName() + ") с \"" +
                    c2.getTitle() + "\" (" + c2.getClass().getSimpleName() + ")");
            System.out.println("Совет: Сравнивайте товары одной категории!");
            return;}

        System.out.println("\nРезультат сравнения:");
        System.out.println("Товар 1: " + c1.getTitle() + " — " + c1.getPrice() + "₽");
        System.out.println("Товар 2: " + c2.getTitle() + " — " + c2.getPrice() + "₽");
        System.out.println("Равны? " + c1.equals(c2));

        if (c1.getPrice() != c2.getPrice()) {
            double diff = Math.abs(c1.getPrice() - c2.getPrice());
            System.out.printf("Разница в цене: %.0f₽ (%s дороже)%n",
                    diff, c1.getPrice() > c2.getPrice() ? c1.getTitle() : c2.getTitle());}}

    // можно сравнивать только совместимые типы
    private static boolean canBeCompared(Category c1, Category c2) {
        // Разрешаем сравнение внутри одной иерархии
        if (c1.getClass() == c2.getClass()) return true;
        // Electronic и его подклассы (MobileDevice) можно сравнивать между собой
        if (c1 instanceof Electronic && c2 instanceof Electronic) return true;
        // GardenItem только с GardenItem
        if (c1 instanceof GardenItem && c2 instanceof GardenItem) return true;
        // Категории можно сравнивать только с категориями
